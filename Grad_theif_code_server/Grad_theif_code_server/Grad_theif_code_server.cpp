...
int main(void)
{
	...
	//Key constants
	const char IP_SERV[] = "192.168.31.155"; // Enter local Server IP address
	...
	while (o == 0);
	{
		...
		int a = recv(ClientConn, servBuff.data(), servBuff.size(), 0);
		if ((servBuff[5] != 5) || (servBuff[7] != 7) && (servBuff[9] != 9))
		{
			printf("Собеседник:");
			for (int i = 0; i++; i <= servBuff.size()-1)
			{
				printf("%c", servBuff[i]);
			}
			printf("\n");
		...
		if ((clientBuff[0] = '/') && (clientBuff[0] = 'e') && (clientBuff[0] = 'x')) { o = 1; }
		a = send(ClientConn, clientBuff.data(), 100, 0);
